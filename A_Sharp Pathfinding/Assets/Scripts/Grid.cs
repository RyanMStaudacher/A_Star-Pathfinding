...
public class Grid : MonoBehaviour
{
    ...
    private void CreateGrid()
    {
        ...
        for (int x = 0; x < gridSizeX; x++)
        {
            ...
            {
                Vector3 gridPoint = gridBottomLeft + Vector3.right * (x * nodeDiameter + nodeRadius) + Vector3.forward * (y * nodeDiameter + nodeRadius);
                bool walkable = !(Physics.CheckSphere(gridPoint, nodeRadius, unwalkableMask));
                grid[x, y] = new Node(walkable, gridPoint);
            }
        }
    }
